...
function combineAndModifyKeywords(otherData, modsData) {
    ...
    const otherTitles = otherData.map(item => [`${item.title} СУМ, ${item.title} українською`, `${item.title} СУМ`, `${item.title}`, `скачать ${item.title}`, `завантажити ${item.title}`, `мод ${item.title}`]);
    const modsTitles = modsData.map(item => [`${item.title} СУМ, ${item.title} українською`, `${item.title} СУМ`, `${item.title}`, `скачать ${item.title}`, `завантажити ${item.title}`, `мод ${item.title}`],);
